...
C_G = nx.erdos_renyi_graph(1000,0.03)
...
f.close()
'''
C_G = nx.random_regular_graph(30,500)
P_G = nx.random_regular_graph(10,500)
...
f=open("数据集/synthetic1_node.text",'w',encoding='utf-8')
for node in C_G.nodes():
    f.writelines(str(node)+'\n') #将边信息存入text中，概念图构建完毕
f.close()

f=open("数据集/synthetic1_concept_edge.text",'w',encoding='utf-8')
for edge in C_G.edges():
    f.writelines(str(edge[0])+'*'+str(edge[1])+'\n') #将边信息存入text中，概念图构建完毕
f.close()

f=open("数据集/synthetic1_physical_edge.text",'w',encoding='utf-8')
for edge in P_G.edges():
    f.writelines(str(edge[0])+'*'+str(edge[1])+'\n') #将边信息存入text中，概念图构建完毕
f.close()
'''